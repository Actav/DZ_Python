...
def find_indexes(lst, min_val, max_val):
    """
    Функция ищет индексы элементов списка, которые находятся в диапазоне от
    min_val до max_val включительно

    :param lst: список чисел
    :param min_val: минимальное значение диапазона
    :param max_val: максимальное значение диапазона
    :return: список кортежей (индекс, значение)
    """
    return [(i, x) for i, x in enumerate(lst) if min_val <= x <= max_val]


lst1 = [-5, 9, 0, 3, -1, -2, 1, 4, -2, 10, 2, 0, -9, 8, 10, -9, 0, -5, -5, 7]
print(find_indexes(lst1, 2, 10))
