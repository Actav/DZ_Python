...
```(*)``` **Усложнение.** Для формирования списка внутри функции
...
def arithmetic_progression(_a1: int, _d: int, _n: int) -> list[int]:
    return [_a1 + (i - 1) * _d for i in range(1, _n + 1)]


mesage = "Введите через пробел первый элемент прогрессии, " \
         "разность и количество элементов: "
a1, d, n = map(int, input(mesage).split())
